...
Claude Next.js Skills is a POC bundle of automations aimed at seeing if creating skills quickly could improve the Next.js eval scores published by Vercel at https://nextjs.org/evals. These skills have been updated to support Next.js 16 with all the latest breaking changes and patterns.
...
This repository has been updated to include Next.js 16-specific patterns and breaking changes:

### Breaking Changes Covered:
- **Proxy instead of Middleware**: `middleware.ts` → `proxy.ts` with `export function proxy()`
- **Fully Async APIs**: `cookies()`, `headers()`, `params`, and `searchParams` must be awaited
- **React `use` Hook**: For unwrapping async params and searchParams in components
- **Node.js Runtime**: Proxy runs in Node.js instead of Edge runtime

### New Skills Added:
- `nextjs-16-proxy-patterns`: Patterns for Next.js 16 proxy functionality
- `nextjs-16-async-apis`: Fully async API patterns for cookies, headers, params, and searchParams

### Updated Skills:
- All existing skills have been updated to reflect Next.js 16 changes
- Proper async patterns for all dynamic APIs
- Updated file naming conventions
- Configuration changes (e.g., `skipMiddlewareUrlNormalize` → `skipProxyUrlNormalize`)
