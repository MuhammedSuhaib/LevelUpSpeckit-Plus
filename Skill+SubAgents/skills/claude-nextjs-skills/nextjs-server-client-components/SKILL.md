...
Provide comprehensive guidance for choosing between Server Components and Client Components in Next.js App Router (Next.js 16+), including cookie/header access, searchParams handling, pathname routing, and React's 'use' API for promise unwrapping.

## Next.js 16 Specific Changes

⚠️ **BREAKING CHANGE NOTICE**: In Next.js 16, all Dynamic APIs that rely on runtime information are now **asynchronous only**:
- `cookies()` function returns a Promise and requires `await`
- `headers()` function returns a Promise and requires `await`
- `params` in layout.js, page.js and route.js must be awaited
- `searchParams` in page.js must be unwrapped with React's `use` hook

**Params and SearchParams in Next.js 16:**
```typescript
// Next.js 16+: Use with React's 'use' hook in components
